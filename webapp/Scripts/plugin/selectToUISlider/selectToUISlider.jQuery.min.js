jQuery.fn.selectToUISlider = function (a) {
      ...
      if (e.eq(0).find("optgroup").size() > 0) {
        ...
        return (
          e
            ...
            .each(function (b) {
              (a[b] = {}),
                (a[b].label = jQuery(this).attr("label")),
                (a[b].options = []),
                jQuery(this)
                  .find("option")
                  .each(function () {
    ...
    j = {
      ...
      slide: function (a, b) {
        var d = jQuery(b.handle),
          e = c(b.value);
        d.attr("aria-valuetext", e)
          .attr("aria-valuenow", b.value)
  ...
  (f.sliderOptions = a ? jQuery.extend(j, a.sliderOptions) : j),
    e.bind("change keyup click", function () {
  ...
  if (
    (e.each(function (a) {
      ...
      0 == f.tooltip && (b = ' style="display: none;"'),
        jQuery(
          '<a href="#" tabindex="0" id="' +
            g[a] +
            '" class="ui-slider-handle" role="slider" aria-labelledby="' +
            h +
            '" aria-valuemin="' +
            f.sliderOptions.min +
            '" aria-valuemax="' +
            f.sliderOptions.max +
            '" aria-valuenow="' +
            f.sliderOptions.values[a] +
            '" aria-valuetext="' +
            c(f.sliderOptions.values[a]) +
            '" ><span class="screenReaderContext">' +
            e +
            '</span><span class="ui-slider-tooltip ui-widget-content ui-corner-all"' +
            b +
            '><span class="ttContent"></span><span class="ui-tooltip-pointer-down ui-widget-content"><span class="ui-tooltip-pointer-down-inner"></span></span></span></a>'
        )
          .data("handleNum", a)
          .appendTo(k);
    }),
    ...
    var l = 0,
      m = k
        .append(
          '<dl class="ui-slider-scale ui-helper-reset" role="presentation"></dl>'
        )
        .find(".ui-slider-scale:eq(0)");
    jQuery(i).each(function (a) {
      m.append(
        '<dt style="width: ' +
          (100 / i.length).toFixed(2) +
          "%; left:" +
          ((a / (i.length - 1)) * 100).toFixed(2) +
          '%"><span>' +
          this.label +
          "</span></dt>"
      );
  ...
  for (
    var n = Math.max(1, Math.round(h.length / f.labels)), o = 0;
    o < h.length;
    o += n
  )
    h.length - o > n &&
  ...
  k.find(".ui-slider-scale dt").each(function (a) {
    jQuery(this).css({ left: ((100 / i.length) * a).toFixed(2) + "%" });
  }),
    k
      .insertAfter(jQuery(this).eq(this.length - 1))
      .slider(f.sliderOptions)
  ...
  return (
    b(p)
      ? jQuery(p).each(function (a) {
          k.find(".ui-slider-tooltip .ttContent").eq(a).text(c(this));
        })
      : k.find(".ui-slider-tooltip .ttContent").eq(0).text(c(p)),
    this
  );
};
