  ...
  function c(a, b, c) {
    ...
    if (k)
      for (
        ...
        switch (h.type) {
          case g:
            if (!a || ("data" == h.name && b && !a.hasAttribute("data")))
              continue;
            var l = this.getValue();
            f || (e && l === h["default"])
              ? a.removeAttribute(h.name)
              : a.setAttribute(h.name, l);
            break;
          case o:
            if (!a) continue;
            l = this.getValue();
          ...
          case i:
            ...
            l = this.getValue();
            f || (e && l === h["default"])
              ? b.removeAttribute(h.name)
              : b.setAttribute(h.name, l);
        }
      }
  }
  for (
    var g = 1,
      ...
      n = {
        id: [{ type: g, name: "id" }],
        classid: [{ type: g, name: "classid" }],
        codebase: [{ type: g, name: "codebase" }],
        pluginspage: [{ type: i, name: "pluginspage" }],
        src: [
          { type: o, name: "movie" },
          { type: i, name: "src" },
          { type: g, name: "data" },
        ],
        name: [{ type: i, name: "name" }],
        align: [{ type: g, name: "align" }],
        class: [
          { type: g, name: "class" },
          { type: i, name: "class" },
        ],
  ...
  CKEDITOR.dialog.add("flash", function (a) {
    var g = !a.config.flashEmbedTagOnly,
      ...
      f =
        ...
        '<br><div id="cke_FlashPreviewLoader' +
        CKEDITOR.tools.getNextNumber() +
        '" style="display:none"><div class="loading">&nbsp;</div></div><div id="cke_FlashPreviewBox' +
        CKEDITOR.tools.getNextNumber() +
      ...
      onShow: function () {
        ...
        if (
          ...
          if ("cke:object" == d.getName()) {
            ...
            for (
              var d = h.getElementsByTag("param", "cke"), g = 0, i = d.count();
              g < i;
              g++
            ) {
              var f = d.getItem(g),
