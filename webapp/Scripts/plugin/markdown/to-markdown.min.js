var toMarkdown = function (a) {
  function b(a, b) {
    var c =
      ...
      e = "";
    return (e =
      "string" == typeof b.replacement
        ? a.replace(d, b.replacement)
        : a.replace(d, function (a, c, d, e) {
  ...
  function d(a) {
    return (
      (a = a.replace(/<(ul|ol)\b[^>]*>([\s\S]*?)<\/\1>/gi, function (a, b, c) {
        var d = c.split("</li>");
  ...
  for (
    ...
    if ("string" == typeof g[h].patterns)
      ...
      for (var j = 0, k = g[h].patterns.length; k > j; j++)
        a = b(a, {
          tag: g[h].patterns[j],
          replacement: g[h].replacement,
          type: g[h].type,
  ...
  for (var l = /<(ul|ol)\b[^>]*>(?:(?!<ul|<ol)[\s\S])*?<\/\1>/gi; a.match(l); )
    a = a.replace(l, function (a) {
      return d(a);
    });
  for (
    var m = /<blockquote\b[^>]*>((?:(?!<blockquote)[\s\S])*?)<\/blockquote>/gi;
    a.match(m);
