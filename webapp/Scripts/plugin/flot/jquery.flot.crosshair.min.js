  ...
  function b(a) {
    ...
    function c(b) {
      if (!d.locked) {
        ...
        var c = a.offset();
        (d.x = Math.max(0, Math.min(b.pageX - c.left, a.width()))),
          (d.y = Math.max(0, Math.min(b.pageY - c.top, a.height()))),
          a.triggerRedrawOverlay();
      }
    }
    var d = { x: -1, y: -1, locked: !1 };
    (a.setCrosshair = function (b) {
      if (b) {
        var c = a.p2c(b);
        (d.x = Math.max(0, Math.min(c.left, a.width()))),
          (d.y = Math.max(0, Math.min(c.top, a.height())));
      } else d.x = -1;
      a.triggerRedrawOverlay();
    }),
      (a.clearCrosshair = a.setCrosshair),
      (a.lockCrosshair = function (b) {
      ...
      a.hooks.drawOverlay.push(function (a, b) {
        ...
        if (c.mode) {
          ...
          if ((b.save(), b.translate(e.left, e.top), -1 != d.x)) {
            var f = a.getOptions().crosshair.lineWidth % 2 ? 0.5 : 0;
            if (
