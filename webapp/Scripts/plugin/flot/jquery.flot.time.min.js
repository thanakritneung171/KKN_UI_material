!(function (a) {
  ...
  function f(d) {
    d.hooks.processOptions.push(function (d, f) {
      a.each(d.getAxes(), function (a, d) {
        var f = d.options;
        "time" == f.mode &&
          ((d.tickGenerator = function (a) {
            var c = [],
              d = e(a.min, f),
              g = 0,
              i =
                (f.tickSize && "quarter" === f.tickSize[1]) ||
                (f.minTickSize && "quarter" === f.minTickSize[1])
                  ? k
                  : j;
            null != f.minTickSize &&
              (g =
                "number" == typeof f.tickSize
                  ? f.tickSize
                  : f.minTickSize[0] * h[f.minTickSize[1]]);
            for (
              var l = 0;
              l < i.length - 1 &&
              !(
                a.delta <
                  (i[l][0] * h[i[l][1]] + i[l + 1][0] * h[i[l + 1][1]]) / 2 &&
                i[l][0] * h[i[l][1]] >= g
              );
              ++l
            );
            ...
            if ("year" == n) {
              if (null != f.minTickSize && "year" == f.minTickSize[1])
                ...
                var o = Math.pow(
                    10,
                    Math.floor(Math.log(a.delta / h.year) / Math.LN10)
                  ),
                  p = a.delta / h.year / o;
            ...
            var r = q * h[n];
            "second" == n
              ? d.setSeconds(b(d.getSeconds(), q))
              : "minute" == n
              ? d.setMinutes(b(d.getMinutes(), q))
              : "hour" == n
              ? d.setHours(b(d.getHours(), q))
              : "month" == n
              ? d.setMonth(b(d.getMonth(), q))
              : "quarter" == n
              ? d.setMonth(3 * b(d.getMonth() / 3, q))
              : "year" == n && d.setFullYear(b(d.getFullYear(), q)),
              d.setMilliseconds(0),
          ...
          (d.tickFormatter = function (a, b) {
            ...
            var g,
              i =
                (b.options.tickSize && "quarter" == b.options.tickSize[1]) ||
                (b.options.minTickSize &&
                  "quarter" == b.options.minTickSize[1]),
  ...
  a.plot.plugins.push({ init: f, options: g, name: "time", version: "1.0" }),
    (a.plot.formatDate = c),
